...
class Student():
    ...
    def all_grades(self, abc):
        query = "SELECT Subject_Name, Total, Grade FROM (SELECT Subject_Name, Pre_Mid, Mid_Term, Post_Mid, Final_Term, CEIL((Pre_Mid + Mid_Term + Post_Mid + Final_Term)/2) AS Total, Percentage, Grade FROM assessment CROSS JOIN grade_table WHERE Student_ID = (SELECT student_ID FROM student WHERE student.Person_ID = (SELECT Person_ID FROM person WHERE Person_ID = '{}'))) Report_Card WHERE Total = Percentage;".format(abc)
